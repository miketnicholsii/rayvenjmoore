            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
              ...
              >
                ...
                <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-semibold text-dark-forest leading-tight mb-6">
                  Guiding You Through{' '}
                  <span className="text-terracotta">Financial Complexity</span>
                </h1>
                <p className="font-body text-lg text-olive leading-relaxed mb-4">
                  Rayven J. Moore is a <span className="text-dark-forest font-medium">Certified Public Accountant</span>, 
                ...
                <p className="font-body text-lg text-olive leading-relaxed">
                  His specialty: <span className="text-dark-forest font-medium">breaking down complex concepts 
              ...
              >
                ...
                <div className="absolute -bottom-6 -left-6 w-48 h-48 border-2 border-terracotta/30 rounded-2xl -z-10" />
              </motion.div>
            </div>
          </div>
        </section>
        ...
        <section className="section-padding bg-cream">
          <div className="container-narrow">
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ duration: 0.6 }}
              className="space-y-6 font-body text-lg text-olive leading-relaxed"
            >
              <p>
        ...
        <section className="section-padding bg-warm-beige/30">
          <div className="container-wide">
            ...
            <div className="hidden lg:block relative">
              {/* Center Line */}
              <div className="absolute left-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-olive/20 via-olive/40 to-olive/20" />
                ...
                {timelineMilestones.map((milestone, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <div className={`bg-cream rounded-2xl p-6 border border-sage/30 hover:border-terracotta/40 transition-all duration-300 hover:shadow-lg ${isLeft ? 'ml-auto' : 'mr-auto'}`}>
                          <div className={`flex items-center gap-3 mb-3 ${isLeft ? 'justify-end' : 'justify-start'}`}>
                            <span className="font-display text-2xl font-bold text-terracotta">{milestone.year}</span>
                          </div>
                          <h3 className="font-display text-xl font-semibold text-dark-forest mb-2">
                            {milestone.title}
                          </h3>
                          <p className="font-body text-sm text-olive leading-relaxed mb-3">
                            {milestone.description}
                          </p>
                          <span className="inline-block font-body text-xs font-semibold tracking-wider uppercase px-3 py-1 rounded-full bg-olive/10 text-olive">
                            {milestone.highlight}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="absolute left-1/2 -translate-x-1/2 z-10">
                        <motion.div 
                          className="w-14 h-14 rounded-full bg-cream border-2 border-olive flex items-center justify-center shadow-lg"
                          whileHover={{ scale: 1.1 }}
                          transition={{ duration: 0.2 }}
                        >
                          <milestone.icon className="w-6 h-6 text-olive" />
                        </motion.div>
                      </div>
            ...
            <div className="lg:hidden relative">
              {/* Left Line */}
              <div className="absolute left-6 top-0 bottom-0 w-px bg-gradient-to-b from-olive/20 via-olive/40 to-olive/20" />
                ...
                {timelineMilestones.map((milestone, index) => (
                  ...
                  >
                    ...
                    <div className="absolute left-0 z-10">
                      <div className="w-12 h-12 rounded-full bg-cream border-2 border-olive flex items-center justify-center shadow-md">
                        <milestone.icon className="w-5 h-5 text-olive" />
                      </div>
                    </div>

                    {/* Content */}
                    <div className="bg-cream rounded-xl p-5 border border-sage/30 flex-grow">
                      <span className="font-display text-xl font-bold text-terracotta">{milestone.year}</span>
                      <h3 className="font-display text-lg font-semibold text-dark-forest mt-1 mb-2">
                        {milestone.title}
                      </h3>
                      <p className="font-body text-sm text-olive leading-relaxed mb-3">
                        {milestone.description}
        ...
        <section className="section-padding bg-cream">
          <div className="container-narrow">
            <motion.div
              ...
              transition={{ duration: 0.6 }}
            >
              <h2 className="font-display text-2xl md:text-3xl font-semibold text-dark-forest mb-8 border-b-2 border-terracotta pb-2 inline-block">
                Core Values
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
                {[
                  ...
                  >
                    <span className="flex-shrink-0 w-10 h-10 rounded-full bg-olive/10 flex items-center justify-center text-terracotta font-display font-semibold">
                      {index + 1}
                    </span>
                    <div>
                      <h3 className="font-display text-lg font-semibold text-dark-forest mb-2">
                        {value.title}
                      </h3>
                      <p className="font-body text-olive">
                        {value.description}
                      </p>
                    </div>
                  </motion.div>
            ...
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ duration: 0.6 }}
              className="mt-20 pt-12 border-t border-sage/30"
            >
              <h2 className="font-display text-2xl md:text-3xl font-semibold text-dark-forest mb-8 border-b-2 border-terracotta pb-2 inline-block">
                Credentials
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
                {[
                  ...
                  <motion.div
                    ...
                    viewport={{ once: true }}
                    transition={{ duration: 0.4, delay: index * 0.1 }}
                    className="bg-warm-beige/50 rounded-xl p-6 text-center hover:bg-warm-beige/70 transition-colors"
                  >
                    <p className="font-display text-4xl font-semibold text-dark-forest mb-2">{stat.value}</p>
                    <p className="font-body text-sm text-olive">{stat.label}</p>
                  </motion.div>
                ))}
              </div>
            </motion.div>
